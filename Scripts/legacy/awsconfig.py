...
with open(args.input_file,'r') as csvfile:
    ...
    for row in reader:
        append_new_line('s3publiclog.txt', 'Starting account Configuration for account {}'.format(row[0]))
        ...
        for configpacks in client.describe_conformance_packs()['ConformancePackDetails']:
            packtogo = configpacks['ConformancePackName']
            print('deleting {0} conformance pack'.format(packtogo))
            client.delete_conformance_pack(ConformancePackName=packtogo)
        append_new_line('s3publiclog.txt', 'Finished S3 Public Configuration for {account}\n'.format(account=row[0]))