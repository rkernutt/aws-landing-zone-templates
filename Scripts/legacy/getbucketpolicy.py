...
with open(args.input_file,'r') as csvfile:
    ...
    for row in reader:
        append_new_line('s3bucketpolicylog.txt', 'Starting account Configuration for account {}'.format(row[0]))
        ...
        for bucket in buckets['Buckets']:
            try:
                ...
                with open('%r-%s.json' % ((row[0]), (bucket['Name'])), 'w', encoding='utf-8') as outfile:
                   outfile.write(bucket_policy)
                   temp = outfile.name
                   print(temp)
                   time.sleep(3)
                   with open('temp', 'rb') as ingress:
                        v = ingress.read()
                        w = json.loads(v)
