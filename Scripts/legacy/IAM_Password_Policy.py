...
with open(args.input_file,'r') as csvfile:
    ...
    for row in reader:
        append_new_line('iampasswordlog.txt', 'Starting account Configuration for account {}'.format(row[0]))
        ...
        append_new_line('iampasswordlog.txt', 'Finished Account Password Policy Configuration for {account}\n'.format(account=row[0]))